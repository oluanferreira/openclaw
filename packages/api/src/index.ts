...
const appRouter = new Hono()
  ...
  .use(delay)
  .use(localize)
  // Global rate limit: 60 req/min per worker (PM2 cluster has 2 workers,
  // so effective total is ~120 req/min across all workers)
  .use(rateLimit(60, 60_000))
  .use(monitor.middleware)
  .route("/status", monitor.routes)
  .route("/auth", authRouter)
  .route("/billing", billingRouter)
  // Deploy endpoint: 5 req/hour per worker (10 total across cluster)
  .use("/openclaw", rateLimit(5, 60 * 60_000))
